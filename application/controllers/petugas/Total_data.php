...
class Total_data extends CI_Controller {
    ...
    public function index()
    {
        if($this->session->userdata('level') == '4') {
            ...
            $user = $this->user_model->listing($username);
            $total_data = $this->data_model->total_data_user2($regional);
            $data = array(  'title' =>  'Total Data',
                            'user' =>  $user,
                            'konfigurasi'   =>  $konfigurasi,
                            'total_data'    =>  $total_data,
                            'isi'   =>  'petugas/total_data/list'
                        );
    ...
    public function cetak()
    {
        if($this->session->userdata('level') == '4') {
            ...
            $this->load->view('petugas/total_data/cetak', $data, false);
        }else{
            $this->session->set_flashdata('warning','Anda belum login');
            redirect(base_url('login'),'refresh');
        }
    ...
    public function cetak_data($id=null)
    {
        ...
        if($this->session->userdata('level') == '4') {
            ...
            $this->load->view('petugas/total_data/cetak_data', $data, false);
        }else{
            $this->session->set_flashdata('warning','Anda belum login');
            redirect(base_url('login'),'refresh');
        }
    ...
    public function cetak_akhir()
    {
        if($this->session->userdata('level') == '4') {
            ...
            $cetak_akhir = $this->data_model->cetak_akhir_user($masa_akhir);
            $konfigurasi = $this->konfigurasi_model->listing();
    ...
    public function cetak_tp()
    {
        if($this->session->userdata('level') == '4') {
            ...
            $cetak_tp = $this->data_model->cetak_tp_user($tanggal_pelaksanaan);
            $konfigurasi = $this->konfigurasi_model->listing();
